			...
			<div class="panel panel-custom">			
			   ...
			   <?php 			   					  
			    	...
			    	foreach ($resultado as $row) { 			    		
			    		...
			    		if($row->USU_AREA == "ADM") {
			    			$area = 'Adminstração';
			    		} elseif ($row->USU_AREA == "IND") {
			    			$area = 'Indústria';
			    		} elseif ($row->USU_AREA == "AGR") {
			    			$area = 'Agrícola';
			    		} elseif ($row->USU_AREA == "EPI") {
			    			$area = 'EPIs';
			    		} elseif ($row->USU_AREA == "QUI") {
			    			$area = 'Químicos';
			    		} elseif ($row->USU_AREA == "TAC") {
			    			$area = 'TAC';
			    		} elseif ($row->USU_AREA == "PRJ") {
			    			$area = 'Projeto';
		    			} elseif ($row->USU_AREA == "LUB") {
		    				$area = 'Lubrificantes';
		    			} elseif ($row->USU_AREA == "ADU") {
		    				$area = 'Adubo';
		    			} elseif ($row->USU_AREA == "DIE") {
		    				$area = 'Diesel';
		    			} elseif ($row->USU_AREA == "DFS") {
		    				$area = 'Defensivo';
		    			} elseif ($row->USU_AREA == "LAB") {
		    				$area = 'Laboratório';
		    			} elseif ($row->USU_AREA == "AUT") {
		    				$area = 'Automotivo';
		    			} elseif ($row->USU_AREA == "PNE") {
		    				$area = 'Pneu';
		    			} elseif ($row->USU_AREA == "SEG") {
		    				$area = 'Segadeira';
			    		} elseif ($row->USU_AREA == null or $row->USU_AREA = 'DEF') {
			    			$area = '';
			    		}
			    ...
			    ?>	
					    	<tr class="info">
					    		<td colspan="14"><strong>Unidade: <?php echo  $row->USU_FILIAL.' - '.$row->SIGFIL.' '.$row->USU_INSTAN ?></strong></td>
					    	</tr>
				<?php
			    		}  
			    ?>
							<tr>
								<td><input type="hidden" name="chek_ger[]" value="<?php echo $row->USU_CODEMP.$row->USU_FILIAL.$row->USU_NUMOCP.$row->USU_ID; ?>" id="chek_ger[]"	/></td>																												
								...
								<td class="text-left"><?php echo utf8_decode($row->USU_OBS) ?></td>
